...
sub generate_request_type_parser_h($)
{
    my ( $file_ref ) = @_;
...
sub generate_request_type_parser_cpp_body__to_make_pair($)
{
    my $name = shift;
...
sub generate_request_type_parser_cpp_body($)
{
    my ( $file_ref ) = @_;
    ...
    {
        $res = $res . generate_request_type_parser_cpp_body__to_make_pair( $_->{name} ) . ",\n";
    }

    return main::tabulate( main::tabulate( $res ) );
}

sub generate_request_type_parser_cpp($)
{
    my ( $file_ref ) = @_;
...
"    static const Map m =\n" .
"    {\n" .
    generate_request_type_parser_cpp_body( $file_ref ) .
...
"}\n";
    ...
    my $res = to_body( $$file_ref, $body, "", [ "request_type_parser" ], [ "map" ] );
